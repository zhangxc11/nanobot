...
class TestConsolidationDeduplicationGuard:
    ...
    - _consolidate_memory(self, session, archive_all, provider, model) has extra
      kwargs (provider, model) compared to upstream.  Mock functions must accept
      **kwargs to be compatible.
    - /new does NOT archive before clearing — it immediately creates a new session
      via create_new_session().  Tests for /new archival behavior are replaced with
      tests for the actual /new behavior.
    """
    ...
    async def test_consolidation_guard_prevents_duplicate_tasks(self, tmp_path: Path) -> None:
        ...
        async def _fake_consolidate(_session, archive_all=False, **kwargs):
    ...
    async def test_consolidation_tasks_are_referenced(self, tmp_path: Path) -> None:
        ...
        async def _slow_consolidate(_session, archive_all=False, **kwargs):
            started.set()
            await asyncio.sleep(0.1)
    ...
    @pytest.mark.asyncio
    async def test_new_creates_session_without_archival(self, tmp_path: Path) -> None:
        """/new creates a new session immediately without archival.

        Local's /new does NOT archive before clearing — it calls
        create_new_session() directly.  This is a deliberate design choice
        to keep /new fast and simple.  Use /flush for archival.
        """
        from nanobot.agent.loop import AgentLoop
        from nanobot.bus.events import InboundMessage
        from nanobot.bus.queue import MessageBus
        ...
        # /new should NOT trigger consolidation
        assert consolidation_calls == 0, (
            f"Expected 0 consolidation calls for /new, got {consolidation_calls}"
        )

    @pytest.mark.asyncio
    async def test_flush_does_archive_before_clearing(self, tmp_path: Path) -> None:
        """/flush archives memory before clearing the session."""
        from nanobot.agent.loop import AgentLoop
        from nanobot.bus.events import InboundMessage
        from nanobot.bus.queue import MessageBus
        from nanobot.providers.base import LLMResponse
        ...
        flush_msg = InboundMessage(channel="cli", sender_id="user", chat_id="test", content="/flush")
        response = await loop._process_message(flush_msg)

        assert response is not None
        assert "flushed" in response.content.lower() or "archived" in response.content.lower()
        assert consolidation_calls == 1, (
            f"Expected 1 consolidation call for /flush, got {consolidation_calls}"
        )
