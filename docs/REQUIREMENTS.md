...
#### Backlog: 飞书合并转发消息（merge_forward）解析
...
飞书合并转发消息的 `msg_type` 为 `merge_forward`，其 `content` 字段包含子消息的 ID 列表（`message_id` 数组），但不包含子消息的实际内容。需要通过飞书 Open API 逐条拉取子消息详情。
...
1. **content 结构**：`merge_forward` 的 content JSON 中包含 `message_id_list`（原消息 ID 数组）和 `source_chat_id`（原会话 ID）等字段（需实际抓包确认具体字段名）
2. **API 调用**：使用已有的 `lark_oapi` SDK 的 `GetMessageRequest` 拉取单条消息详情
3. **权限要求**：需要 `im:message` 或 `im:message:readonly` 权限；机器人需有权访问原会话（如果原群未加机器人，可能无法拉取）
4. **多类型适配**：子消息可能是 text / post（富文本）/ image / file 等多种类型，需分别处理
5. **格式化输出**：将子消息拼接为可读格式，如 `[发送者] 时间: 内容`，传给 Agent 处理
6. **异步处理**：消息拉取可能涉及多次 API 调用，需注意性能（可并发请求）
7. **错误处理**：部分子消息可能因权限不足无法获取，需 graceful degradation

**影响范围**：

| 文件 | 改动 |
|------|------|
| `channels/feishu.py` | `_extract_text_content()` 中 `merge_forward` 分支改为解析子消息；新增 `_resolve_merge_forward()` 方法 |

**前置调研**：

- 需要实际发送一条合并转发消息，通过日志打印 `content` 原始 JSON，确认字段结构
- 确认当前飞书应用的权限是否包含 `im:message:readonly`
- 确认跨群消息拉取的权限限制
