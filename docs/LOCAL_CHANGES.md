...
| `15b22b6` | `providers/pool.py` | fix | ProviderPool.chat() 添加 reasoning_effort 参数 |
| TBD | `providers/pool.py`, `agent/subagent.py`, `tests/test_provider_pool.py` | improve | ProviderPool **kwargs 透传 + subagent 条件传递 + 接口一致性测试 |
...
## Phase 27: ProviderPool **kwargs 透传 + 接口一致性防护

> 日期: 2026-03-07
> 来源: `reasoning_effort` bug 复盘（3月2日 merge upstream 后 ProviderPool 缺参数，导致 subagent 全部失败）

### 问题回顾

Phase 22 merge upstream 时，`base.py` 和 `litellm_provider.py` 新增了 `reasoning_effort` 参数，但 `pool.py` 是 local 独有代码，merge 不会自动更新。Phase 26 的 subagent `_chat_with_retry()` 直接传 `reasoning_effort=self.reasoning_effort` 给 `provider.chat()`，当 provider 是 ProviderPool 时报错 `unexpected keyword argument`。

### 27a: ProviderPool.chat() 改为 **kwargs 透传

**文件**: `providers/pool.py`

将 `chat()` 签名从显式参数列表改为 `(self, messages, **kwargs)`，透传所有参数给底层 provider。这样 upstream 添加任何新参数时，ProviderPool 无需修改。

```python
# Before (fragile — must manually sync with base.py):
async def chat(self, messages, tools=None, model=None, max_tokens=4096,
               temperature=0.7, reasoning_effort=None) -> LLMResponse:
    ...
    return await provider.chat(messages, tools=tools, model=self._active_model, ...)

# After (forward-compatible):
async def chat(self, messages, **kwargs) -> LLMResponse:
    kwargs["model"] = self._active_model  # always override
    return await provider.chat(messages, **kwargs)
```

### 27b: SubagentManager._chat_with_retry() 条件传递

**文件**: `agent/subagent.py`

将 `reasoning_effort` 从直接传递改为条件传递（与 `agent/loop.py` 的 `_chat_with_retry` 一致），避免向不支持该参数的 provider 传递 None 值：

```python
# Before:
return await self.provider.chat(
    messages=messages, ..., reasoning_effort=self.reasoning_effort,
)

# After:
kwargs = dict(messages=messages, tools=..., model=..., ...)
...
**文件**: `tests/test_provider_pool.py`

新增 `TestProviderInterfaceConsistency` 测试类（3 项测试）：

1. `test_pool_chat_accepts_base_params`: 验证 ProviderPool 使用 **kwargs（前向兼容）
2. `test_litellm_provider_chat_matches_base`: 验证 LiteLLMProvider 签名与 base 一致
3. `test_custom_provider_chat_matches_base`: 验证 CustomProvider 签名与 base 一致

新增回归测试（2 项）：
4. `test_chat_passes_reasoning_effort`: 验证 reasoning_effort 正确透传
5. `test_chat_forwards_unknown_kwargs`: 验证未来新参数也能透传

### 27d: Merge 后必检清单

**文件**: `docs/LOCAL_CHANGES.md`

在文档末尾新增"Merge 后必检清单"章节，列出 4 个检查项：
1. LLMProvider 接口一致性（运行签名测试）
2. SubagentManager 参数透传
3. SDK AgentRunner 初始化同步
4. Web-chat Gateway/Worker 兼容性

### 测试
- `tests/test_provider_pool.py`: 36 项测试（+5 新增）
- 全量 384 passed
...
> 每次从 `main` merge/rebase 到 `local` 后，按以下清单逐项检查。

### 1. LLMProvider 接口一致性

upstream 可能在 `base.py` 的 `LLMProvider.chat()` 中添加新参数。所有实现类必须同步更新。

**检查命令**:
```bash
python -m pytest tests/test_provider_pool.py::TestProviderInterfaceConsistency -v
```

**涉及文件**:
| 文件 | 角色 | 说明 |
|------|------|------|
| `providers/base.py` | 接口定义 | upstream 维护 |
| `providers/litellm_provider.py` | 实现 | upstream 维护 |
| `providers/custom_provider.py` | 实现 | upstream 维护 |
| `providers/pool.py` | 代理（**kwargs 透传） | **local 独有**，已改为 `**kwargs` 透传，理论上不需要手动更新 |

**历史教训**: 2026-03-02 merge upstream 后 `pool.py` 缺少 `reasoning_effort` 参数，导致 subagent 全部失败（直到 3-07 才定位）。

### 2. SubagentManager 参数透传

`agent/subagent.py` 的 `_chat_with_retry()` 使用条件传递模式（`if param is not None: kwargs[key] = param`）。
如果 upstream 在 `LLMProvider.chat()` 新增参数且 `AgentLoop` 也传递了该参数，需要检查 `SubagentManager.__init__` 和 `_chat_with_retry()` 是否也需要同步。

### 3. SDK AgentRunner 初始化

`sdk/runner.py` 的 `from_config()` 镜像 `cli/commands.py` 的初始化逻辑。
upstream 修改 `commands.py` 的 provider/loop 初始化时，需要同步更新 `runner.py`。

### 4. Web-chat Gateway/Worker 兼容性

如果 upstream 修改了 `AgentLoop` 构造函数签名或 `AgentCallbacks` 协议，需要同步更新 web-chat 仓库的 `gateway.py` 和 `worker.py`。
