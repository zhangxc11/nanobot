...
- [ ] **T38.7** `tests/test_spawn_follow_up.py` — 测试用例
  - follow_up inject（运行中 subagent）
  - follow_up resume（已结束 subagent）
  - 鉴权失败（不属于自己的 subagent）
  - 鉴权失败（不存在的 task_id）
  - resume persist=False 报错
  - inject drain 多条消息
  - resume 后再次结束可再次 follow_up
  - SpawnTool 参数 schema 包含 follow_up
- [ ] **T38.8** 全量回归测试
- [ ] **T38.9** `docs/REQUIREMENTS.md` — 新增 §36 章节
- [ ] **T38.10** `docs/ARCHITECTURE.md` — 新增 §36 章节
- [ ] **T38.11** DEVLOG.md 记录结果
