...
- [ ] **T12.2** `session/manager.py` — 新增 `archive_session()` 和路由映射
  - `archive_session(key)`: 将当前 session 文件改名为带时间戳的归档文件
  - `get_routed_session_key(channel, chat_id)`: 获取当前路由的 session_key
  - `create_new_session_for(channel, chat_id)`: 创建新 session，更新路由映射
  - 路由映射持久化到 `sessions/_routing.json`

- [ ] **T12.3** `agent/loop.py` — 新增 `/new` 处理逻辑（gateway + CLI）
  - Gateway: 创建新 session_key，更新路由映射
  - CLI: 归档当前 session 文件，创建新空 session
  - 更新 session_key 解析逻辑以支持路由映射

- [ ] **T12.4** web-chat 前端 — `/new` 改为前端拦截
  - 创建新 session + 切换（不发送到后端）
  - `/flush` 发送到后端（原 `/new` 的后端行为）
