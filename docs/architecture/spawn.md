...
## §二十四 Subagent detail_logger 接入 (§48)

### 设计

SubagentManager 新增 `detail_logger` 参数，在 `_run_subagent()` 中 LLM 调用后
调用 `detail_logger.log_call()` 记录完整请求/响应。

AgentLoop 在构造 SubagentManager 时传入 `detail_logger`。
对于外部传入的 SubagentManager（§40 单例模式），AgentLoop 检查并继承 detail_logger。

### 数据流

```
AgentLoop.detail_logger
  ↓ 构造时传入 / 运行时继承
SubagentManager.detail_logger
  ↓ _run_subagent() 中 LLM 调用后
LLMDetailLogger.log_call()
  ↓
...
## §二十六 Subagent announce 标记修正 + 闭合标签 (§48)

### 问题

§45 的 `<!-- nanobot:system -->` 标记放在消息开头，导致 final_text 也在标记内，
被前端截断。
...
Review this result in the context of your current session. ...
<!-- /nanobot:system -->
```
